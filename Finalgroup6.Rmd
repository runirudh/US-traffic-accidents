...
# loading data agn n clean up.
US_Accidents <- read.csv("~/Downloads/Accidents Data CSV.csv",stringsAsFactors=FALSE, header=T)
...
ggpairs(US_Accidents, 
        ...
        columns = c("Side", "ZipcodeLevels", "Timezone", "Time.Interval","Temperature.F.","Wind_Chill.F.","Wind_Speed.mph.","Humidity...",
                    "Pressure.in.","Visibility.mi.", "timediff.min", "Severity"))
#Column 'State' has more levels (49) than the threshold (15) allowed, so not added.
```
wind chill & temp correlated. 
```{r}
# Too many features. Takes too long to run
#g1 <- glm(Severity ~ .,family=binomial(link='logit'), data = US_Accidents)
#summary(g1)
#step(g1)


g2 <- glm(Severity ~ Pressure.in. + Wind_Chill.F. + State + Side + Humidity... + Temperature.F. + Wind_Speed.mph. + Time.Interval + Timezone + Zipcode + Visibility.mi. ,family=binomial(link='logit'), data = US_Accidents)

summary(g2)

```
State level 0 = Alabama.
Coefs are interpreted as state changes from alabama, p of Accident severity increasing by coef value
...
Wind_Chill and temperature are correlated, so dropping Temperature off, as stepwise in previous model took it out as well. Removing humidity as well as it got removed in the first iteration of stepwise regression on above big model.
```{r}
...
# manually taking out variables that don't reduce deviance alot and performing anova. 
...
```{r}
# model w/out Timezone as its coefs are not significant for Mountain, Pacific dummy variables for some cases. 
...
#cv.g2.step <- DAAG::CVbinary(g2.step) #er: factor State has new levels WY, model sees level #Wyoming in test fold but not in train fold

cv.g5 <- DAAG::CVbinary(g5)
cv.g5best <- DAAG::CVbinary(g5best)

# same accuracy here. choose less complex model.
```
...
# data is already randomly sampled, sampling again, selecting 70-30 % train-test
dt = sort(sample(nrow(US_Accidents), nrow(US_Accidents)*.7))
train<-US_Accidents[dt,]
test<-US_Accidents[-dt,]
table(is.na(train))
table(is.na(test))
...
fitted.results <- predict(g5train,newdata= subset(test,select=c(6,14,49,20,50,27)), type='response')
...
p <- predict(g5train, newdata=subset(test,select=c(6,14,49,20,50,27)), type="response")
...
# our model does not hv very good predicting ability it seems. 
...
# reject null. our residuals are autocorrelated. but the value isn't very big
```
```{r}
final<- coef(g5best)
exp(final)
...
# we see some non constant variance in these plots, but since  
